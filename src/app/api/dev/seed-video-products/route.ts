...
export async function POST(): Promise<Response> {
  ...
  // Son 2 video ve son 2 ürünü al
  const { data: videos } = await supabase.from('videos').select('id').order('created_at', { ascending: false }).limit(2)
  const { data: products } = await supabase.from('products').select('id').order('created_at', { ascending: false }).limit(2)
  if (!videos?.length || !products?.length) {
    return NextResponse.json({ error: 'Videos or products missing. Seed them first.' }, { status: 400 })
  }
